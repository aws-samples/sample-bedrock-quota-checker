...
**Source access:** this repository is hosted on AWS GitLab and requires authorized access with SSH authentication. AWS console credentials alone do not provide that access. External customers need an approved source archive or a repository they can access. To use a source archive in CloudShell, choose **Actions → Upload file**, extract the archive, and run the installation commands from its project directory.
